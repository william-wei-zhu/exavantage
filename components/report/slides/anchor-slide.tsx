...
import { CompanyLogo, Ribbon, PRINT_EXACT, clip } from "./bits";
import { SlideFrame } from "./slide-frame";

/** The anchor: the platform candidate to build the roll-up around. */
export function AnchorSlide({ report, firm, lens, full, page }: SlideProps & { page?: PageInfo }) {
  const t = firm.theme;
      ...
      {a ? (
        <>
          <div className="relative overflow-hidden rounded-xl p-5" style={{ background: t.surface, ...PRINT_EXACT }}>
            ...
            <p className="relative mt-3.5 line-clamp-3 max-sm:line-clamp-none max-w-3xl text-[16px] leading-snug" style={{ color: `${t.ink}c8` }}>{clip(full, a.why, 30)}</p>
          </div>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Panel title="What it brings" t={t}>{clip(full, a.brings, 26)}</Panel>
            <Panel title="What it needs (the add-ons)" t={t}>{clip(full, a.needs, 26)}</Panel>
          </div>
        </>
      ) : (
...
function Panel({ title, t, children }: { title: string; t: SlideProps["firm"]["theme"]; children: React.ReactNode }) {
  return (
    <div className="rounded-lg p-4" style={{ background: `${t.primary}0c`, ...PRINT_EXACT }}>
      ...
      <p className="mt-1.5 line-clamp-4 max-sm:line-clamp-none text-[15px] leading-snug" style={{ color: `${t.ink}c8` }}>{children}</p>
    </div>
  );
}
