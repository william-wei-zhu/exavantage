...
export function CoverSlide({ report, firm, lens, page }: SlideProps & { page?: PageInfo }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-h-0 flex-1 flex-col justify-between overflow-hidden px-8 py-5 sm:px-12 sm:py-6">
        <div>
          ...
          <h1 className="mt-4 line-clamp-3 max-w-4xl text-[26px] font-bold leading-[1.14] tracking-tight sm:text-[36px]" style={{ fontFamily: t.headingFont, color: t.primary }}>
            {truncateWords(verdict, 24)}
          </h1>
        </div>
        ...
        <div className="mt-5">
          ...
          {thesis?.ask && (
            <div className="mt-4 rounded-md p-4" style={{ background: t.surface, ...PRINT_EXACT }}>
              ...
              <p className="mt-1 line-clamp-2 text-[14px] font-semibold leading-snug" style={{ color: `${t.ink}d8` }}>{truncateWords(thesis.ask, 22)}</p>
            </div>
          )}
        </div>
      </div>
