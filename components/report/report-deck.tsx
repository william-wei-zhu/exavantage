...
function Toolbar({ firm, report, shareId }: { firm: Firm; report: Report; shareId?: string | null }) {
  ...
  // On phones the four actions wrap into a 2-up grid (full-width buttons so the
  // longer labels never clip); from sm up they collapse to one centered row.
  return (
    <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center">
      <Button variant="outline" size="sm" className="w-full sm:w-auto border-transparent bg-[#DBEAFE] text-[#1D4ED8] hover:bg-[#BFDBFE]" onClick={onRegenerate}>
        <RefreshCw className="h-4 w-4" />
        Regenerate
      </Button>
      {shareId && (
        <Button variant="outline" size="sm" className="w-full sm:w-auto border-transparent bg-[#FEF9C3] text-[#854D0E] hover:bg-[#FEF08A]" onClick={onCopy}>
          {copied ? <Check className="h-4 w-4" /> : <Link2 className="h-4 w-4" />}
          {copied ? "Copied" : "Copy share link"}
        </Button>
      )}
      <Button variant="outline" size="sm" className="w-full sm:w-auto border-transparent bg-[#FEE2E2] text-[#B91C1C] hover:bg-[#FECACA]" onClick={onExport}>
        <Download className="h-4 w-4" />
        Export deck (PDF)
      </Button>
      <Button variant="outline" size="sm" className="w-full sm:w-auto border-transparent bg-[#DCFCE7] text-[#15803D] hover:bg-[#BBF7D0]" onClick={onCsv}>
