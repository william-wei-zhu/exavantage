/**
 * The official Exa wordmark, ported from williamzhu.ai/exa-growth. Letters use
 * `currentColor` so they adapt to light/dark; the geometric mark keeps Exa's
 * brand blue. Size via `className` (e.g. `h-5 w-auto`).
 */
...
/**
 * Just the Exa geometric mark (the blue glyph), for favicons and compact spots.
 * viewBox is cropped to the glyph so it fills its box.
 */
export function ExaMark({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 79 100"
      fill="none"
...
export function ExaVantageLockup({ className }: { className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2.5 ${className ?? ""}`}>
      <ExaLogo className="h-[26px] w-auto text-foreground" />
